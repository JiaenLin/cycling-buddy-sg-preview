...
test('adds the mountain-bike layer: named routes and graded singletrack, its own legend toggle, tappable', async ({ page }) => {
  ...
  // the shipped layer carries the 4 named route=mtb relations plus graded singletrack (Chestnut + Ketam)
  const data = await page.evaluate(async () => (await (await fetch('data/mtb.lines.geojson')).json()).features
    .map(f => ({ kind: f.properties.kind, name: f.properties.name, grade: f.properties.grade })));
  expect(data.filter(f => f.kind === 'route').length).toBe(4);
  expect(data.filter(f => f.kind === 'trail').length).toBeGreaterThanOrEqual(20);
  expect(data.some(f => f.name === 'Gangsa Loop')).toBe(true);
  expect(data.filter(f => f.kind === 'trail').every(f => Number.isInteger(f.grade) && f.grade >= 0 && f.grade <= 4)).toBe(true);
  // the layer reaches Pulau Ubin / Ketam Bike Park in the east, not just the northern reserves
  expect(await page.evaluate(() => MTB_META.bounds[2])).toBeGreaterThan(103.9);
