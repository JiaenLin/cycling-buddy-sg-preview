...
// Mountain-bike line popup — a named route, or a difficulty-graded singletrack segment (mtb:scale).
function mtbPopupHtml(f){
  ...
  const s=Number(p.mtb_scale);
  const grade = s<=0 ? 'beginner' : s===1 ? 'intermediate' : 'advanced';
  return `<b><i class="sw" style="background:${col}"></i>${esc(p.name||'Off-road trail')}</b><span class="pk">Singletrack · ${grade} (mtb:scale ${s}) · ${km.toFixed(1)} km</span>`;
}
